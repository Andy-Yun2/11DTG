...
class Math:
    ...
    def __init__(self, level, q_length):
        """Initialize math question with level and length."""
        self.level = level
    ...
    def pick_question_type(self, level):
        """Pick a type of question based on level."""
        if level >= 5:
    ...
    def non_zero(self, low, high):
        """Return a non-zero random integer in range."""
    ...
    def generate_numbers(self):
        """Generate arithmetic number question."""
        if self.level < 2:
        ...
        if self.level < 6:
            ...
            match choices:
                case "cubic":
                    ...
                    return question_text, answer
                case "quartic":
                    a, b, c, d, e = (self.non_zero(-3, 7) for _ in range(5))
                    op, op1, op2, op3 = tuple(r.choice(["+", "-"]) for _ in range(4))
                    x = self.non_zero(-5, 5)
                    question_text = (
                        f"Find f({x}) if f(x) = {a}x^4 {op} {b}x^3 {op1} {c}x^2 {op2} {d}x {op3} {e}"
                    )
                    question_text = question_text.replace("+ -", "- ").replace("- -", "+ ")
                    answer = eval(
                        f"({a})*({x})**4 {op} ({b})*({x})**3 {op1}({c})*({x})**2 "
                        f"{op2} ({d})*({x}) {op3} {e}"
                    )
                    return question_text, answer
        return None
    ...
    def generate_algebra(self):
        """Generate algebra questions using sympy."""
        x = sp.symbols("x")
        if self.level < 3:
            ...
            solution = sp.solve(sp.Eq(left_expr, b), x)
            return question_text, float(solution[0])
        elif 3 <= self.level < 5:
            a = r.choice([i**2 for i in range(1, 13)])
...
def main(name):
    """Run the main Math Dungeon game loop."""
    enemy_list = [
        Enemy("Novice", 1),
        Enemy("Apprentice", 1),
        Enemy("Beginner", 2),
        ...
        while enemy.hp > 0:
            ...
            try:
                ...
                if round(user_answer, 2) == round(question.answer, 2):
                    ...
                    lives -= 1
                    print(
                        f"Alas! The correct answer was {round(question.answer,2)}. "
                        f"The {enemy.name} strikes back!"
                    )
