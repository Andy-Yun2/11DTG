...
class HighScores:
    ...
    @staticmethod
    def save(game_name, player_name, score):
        """Save a player's score for a specific game."""
        with open(HighScores.FILE, "a") as file:
            file.write(f"{game_name},{player_name},{score}\n")
    ...
    def show(game_name=None):
        ...
        try:
            ...
            print(f"{'Game':<12} | {'Player':<10} | {'Score'}")
            print("-" * 32)
            for line in lines:
                game, name, score = line.strip().split(",")
                if game_name is None or game == game_name:
                    print(f"{game:<12} | {name:<10} | {score}")
